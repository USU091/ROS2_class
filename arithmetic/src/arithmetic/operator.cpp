    ...
    : Node("operator", options)
{
    ...
    rnd_state = get_parameter("random").get_value<bool>();
...
void Operator::timer_callback()
{
    ...
    // 보낼 변수 선언
    /*
    auto request = std::make_shared<interface_exmpl::srv::ArithmeticOperator::Request>();
...
void Operator::random_operator()
{
    ...
    {
        request->arithmetic_operator = arithmeticOperator;
    }
    auto result_future = _service->async_send_request(
        request, std::bind(&Operator::response_callback, this,
                           std::placeholders::_1));
...
void Operator::response_callback(rclcpp::Client<interface_exmpl::srv::ArithmeticOperator>::SharedFuture future)
{
    auto status = future.wait_for(1s);
    if (status == std::future_status::ready)
    {
        //RCLCPP_INFO(get_logger(), "Result of %f", future.get()->arithmetic_result);
    }
    else
    {
